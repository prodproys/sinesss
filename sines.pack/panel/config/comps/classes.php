...
function tabla_chain2($obj,$chain_items=[],$more=[]){


	foreach($chain_items as $ii=>$items_str){
		if(is_array($items_str)){
			foreach($items_str as $iii=>$item_str){
				if($iii!='me'){
					$obj[$items_str['me']][$iii]=$item_str;
				}
			}
			$chain_items2[]=$items_str['me'];
		} else {
			$chain_items2[]=$items_str;
		}
	}

	$chain_items=$chain_items2;
		...
		foreach($items as $item){
			...
			foreach($iitems as $iitem){
				...
				if($obj[$iitem]['archivo']){
					...
					$next_plural       = $obj[$iitem]['nombre_plural'];
					$current_id_foreig = 'id_'.$obj[$chain_items[$ii]]['nombre_singular'];
					$obj[$chain_items[$ii]]['campos']['nombre']['controles'].= "<a href='custom/$next_archivo.php?$current_id_foreig=[id]'>{select count(*) from $next_tabla where $current_id_foreig=[id]} $next_plural</a>\n\n";
